...
python3 skills/tavily-search/scripts/tavily_search.py --query "..." --max-results 5 --include-answer

# stable schema (closer to web_search): {query, results:[{title,url,snippet}], answer?}
python3 skills/tavily-search/scripts/tavily_search.py --query "..." --max-results 5 --format brave

# human-readable Markdown list
python3 skills/tavily-search/scripts/tavily_search.py --query "..." --max-results 5 --format md
```
